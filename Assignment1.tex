...
To construct a line joining $\Vec{A}=\myvec{1\\2}$ and $\vec{B}=\myvec{3\\6}$ 
 and let $\vec n$ be the normal vector then
\begin {center}
...
\end{equation}
\begin{equation}
\myvec{{\vec{A}^T} \\ {\vec{B}^T}} \vec{n} = \myvec{1\\1} \end{equation}
\begin{equation}
\vec{n}=\myvec{\vec{A}^T \\ \vec{B}^T}^{-1}  \myvec{1\\1}
\end{equation}
\\
\begin{equation}
{\myvec{\vec{A}^T \\ \vec{B}^T}}^{-1} = \frac { adj {{\myvec{\vec{A}^T \\ \vec{B}^T}}}} {\det \myvec{\vec{A}^T \\ \vec{B}^T}}
\end{equation}
To  calculate inverse of matrix the determinant of a matrix should not be zero then unique solution exits
adj {\myvec{\vec{A}^T \\ \vec{B}^T}} = \begin{pmatrix} 6 & -2 \\-3 & 1}\end{pmatrix}
\begin{equation}
\det \myvec{{\vec{A}^T} \\ {\vec{B}^T}}=
\begin{vmatrix}1 & 2 \\ 3 & 6\end{vmatrix}}=6-6=0
\end{equation}
\\
Invese of Matrix dose not exist. and Matrix \vec{n} can not be formed.
...
\label{fig:1}
\end{figure}
from (2.0.5) the augmented Matrix is 
\begin {center}
\begin{equation}
\myvec{1 & 2 & 1 \\ 3 & 6 & 1}
\end{equation}
...
\begin{pmatrix}
1 &  2 & 1 \\ 3&  6& 1 
\end{pmatrix} \overset{3r_1-r_2\rightarrow r_2}{\longrightarrow}
\begin{pmatrix} 1 &  2& 1\\0 &  0& -2
\end{pmatrix}
\end{align}
\end{center}
As the elements of the second Row have only one Non Zero element the system of equations have No Solution for $\vec{n} $
\end{document}
